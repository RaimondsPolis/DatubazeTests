...
def iegut_skolotajus():
    cur = conn.cursor()
    cur.execute(
        """SELECT vards, uzvards, id FROM skolotaji ORDER BY skolotaji.uzvards ASC"""
    )
    conn.commit()
    dati = cur.fetchall()
    return dati
...
def iegut_videjas_atzimes():
    ...
    cur.execute(
        ...
        FROM (skoleni LEFT JOIN atzimes ON skoleni.id = atzimes.skolena_id) LEFT JOIN prieksmeti ON prieksmeti.id = atzimes.prieksmeta_id
...
def dzest_skolenu(id):
    ...
    cur.execute(
    f"""
    DELETE FROM skoleni WHERE id="{id}"
    """
    )
    conn.commit()